 ...
 */
 ...
 (function () {
     ...
     * @override
     */
    UI_CUSTOM_TABLE_CLASS.$cache = function(style, cacheSize) {
        UI_LOCKED_TABLE_CLASS.$cache.call(this, style, cacheSize);
        if (chromeVersion) {
            var len = this._aHCells.length;
            this.$$mainWidth += this._aHCells[len - 1].$$borderRightWidth;
            this.$$mainHeight += this._aHCells[len - 1].$$borderBottomWidth;
            this.$$paddingLeft += this._aHCells[len - 1].$$borderBottomWidth;
        }
    };
